...
class InteractiveBrokersExecutionClient(LiveExecutionClient):
    ...
    async def _parse_ib_order_to_order_status_report(self, ib_order: IBOrder) -> OrderStatusReport:
        ...
        # Use permId for external orders (orderId=0 or no orderRef) since it's unique across sessions
        is_external_order = ib_order.orderId == 0 or not ib_order.orderRef
        venue_order_id = (
        ...
        client_order_id = (
            ClientOrderId(f"EXTERNAL-{ib_order.permId}")
            if is_external_order and not ib_order.orderRef
            else ClientOrderId(ib_order.orderRef)
        )
    ...
    def _on_exec_details(
        ...
        # Check if venue_order_id needs to be updated (e.g., order from previous session
        # was loaded with orderId=0 but now we have the real orderId from the execution)
        execution_venue_order_id = VenueOrderId(str(execution.orderId))
        if (
            nautilus_order.venue_order_id is not None
            and nautilus_order.venue_order_id != execution_venue_order_id
            and execution.orderId != 0
        ):
            self._log.info(
                f"Updating venue_order_id for {nautilus_order.client_order_id}: "
                f"{nautilus_order.venue_order_id} -> {execution_venue_order_id}",
            )
            self.generate_order_updated(
                strategy_id=nautilus_order.strategy_id,
                instrument_id=nautilus_order.instrument_id,
                client_order_id=nautilus_order.client_order_id,
                venue_order_id=execution_venue_order_id,
                quantity=nautilus_order.quantity,
                price=nautilus_order.price if hasattr(nautilus_order, "price") else None,
                trigger_price=(
                    nautilus_order.trigger_price
                    if hasattr(nautilus_order, "trigger_price")
                    else None
                ),
                ts_event=self._clock.timestamp_ns(),
